...
Canonical examples: `src/PrstKvsValBuf.bsv` (clean HBM/DDR read+write) and
`src/CuckooHashTable.bsv` (8-channel HBM) in the **kvs_cuckoo** reference design
(pinned in [`../PROVENANCE.md`](../PROVENANCE.md)); bare `src/…` paths below name
files in that repository. The transactors and bus presets are vendored here as
[`../libs/bsv/Axi4Utilities.bsv`](../libs/bsv/Axi4Utilities.bsv) and
[`../libs/bsv/Axi4BusesDefines.bsv`](../libs/bsv/Axi4BusesDefines.bsv); the
in-corpus worked AXI master is
...
(`src/CuckooHbm.bsv`, `src/CuckooHashTable.bsv`). Expose at top as
...
`max_axi_burst = 8`, `axi_beat_nb = 64` (512 bits / 8). See `do_lookup_start` /
`do_lookup_req_more` in `src/PrstKvsValBuf.bsv`.
...
- `max_flight` arg to the transactor sets outstanding depth (e.g.
  `mkAxi4MasterFromTlm(vbuf.ddr_tlm, 32)` in `src/PrstKvsTop.bsv`).
- Slave side mirrors this: `mkAxi4SlaveFromFifoPair` /
  `getTlmRecvFromFifoPair` ([`../libs/bsv/Axi4Utilities.bsv`](../libs/bsv/Axi4Utilities.bsv)); `src/PseudoAxi4Dram.bsv`
  is a model DRAM slave for sim.
