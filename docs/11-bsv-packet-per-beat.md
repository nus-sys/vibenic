...
Canonical example: `src/PrstKvsEthPl.bsv` in the **kvs_cuckoo** reference design
(pinned in [`../PROVENANCE.md`](../PROVENANCE.md)); bare `src/…` paths below name
files in that repository. The header and stream utilities it builds on are
vendored here as [`../libs/bsv/EtherDefines.bsv`](../libs/bsv/EtherDefines.bsv)
and [`../libs/bsv/AxisGetPut.bsv`](../libs/bsv/AxisGetPut.bsv); the in-corpus
worked ingress and egress stages are
