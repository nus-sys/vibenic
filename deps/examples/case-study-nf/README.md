...
**Functionally complete and byte-exact in simulation.** The suite covers every
pure-logic stage in Bluesim, the flow table and the integrated top through
cocotb/Verilator, and an end-to-end byte diff against the numpy golden model
with the counter conservation laws asserted — including under random
backpressure and memory latency. `make check` from the corpus root runs it.
...
One note worth keeping, because the same mistake is easy to repeat: a
long-tracked "~27 % header drop under backpressure" turned out to be a
**testbench artifact**, not a design defect. A hand-written egress collector
drove `tready` itself and sampled in `ReadOnly`, mis-scoring transfers by one
cycle. With `cocotbext-axi`'s `AxiStreamSink` the design is byte-exact and
needed no change. See
