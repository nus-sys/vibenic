...
**MUST: arbitrate explicitly when two producers feed one `Put`.** It compiles;
`bsc` emits **`G0010` — "treated as more urgent than"** — and picks a static
winner, which starves the loser indefinitely under load. Use
[`../libs/bsv/GPMux.bsv`](../libs/bsv/GPMux.bsv). **Read `G0010` on every
build**: a conflict the compiler resolves for you is still a design decision,
just one you did not make.

**MUST NOT: call a value method twice in one rule, even in mutually exclusive
branches.** A value method has one port and the scheduler counts uses, not uses
that can co-occur. Pass the differing input as an argument, call once, select
afterwards — which is also one comparator tree instead of two.
...
## Depth, not just function

Functional testing cannot tell a tree from a chain. These are correct in
simulation and unclosable in silicon, so they are a review obligation.

**MUST NOT: reduce with `foldr`/`foldl` on a wide term set.** A fold is a
*chain* — 512 terms is 512 levels deep. Use `reduceXor` / `reduceOr` /
`reduceAnd` or a `Vector` fold that emits a balanced tree, and restructure the
data so one of them applies (storing a GF(2) matrix row-major turns a
matrix-vector product into `reduceXor(row & v)` per output bit, same cycle
count, correct depth).

**MUST NOT: accumulate through a chained dynamic index.** Scanning every byte
position of a beat into a register with a dynamic index is N conditional updates
each feeding the next. It type-checks fast and then drives `bsc`'s code
generator into tens of gigabytes and minutes of elaboration. Bound the
extraction: a field at a known offset in a known window is a few selects done
once.

**SHOULD: keep match-and-select structures under the width-dependent ceiling** —
about 16 ways at small widths, 4–8 ways at 128 bits or wider. Past that, stage
the logic and account for the cycle. Do the wide arithmetic **once after** the
priority encode, not once per way. Shrinking the table is a legitimate answer if
timing does not close.

If you claim a structure is "about N LUT levels" in a reference architecture,
that claim is about the code as written — check it against the code, or do not
make it.
...
- **Do not skip the post-reset warmup** (`2**htSizeLog2` cycles) before issuing table
...
- [ ] No `foldr`/`foldl` reduction over a wide term set; no chained dynamic-index
      accumulation.
- [ ] Every `G0010` in the build log accounted for.
- [ ] Any `pack`/`unpack` between a struct and a `Vector` reverses deliberately
      — element 0 is the LSBs, field 1 is the MSBs.
