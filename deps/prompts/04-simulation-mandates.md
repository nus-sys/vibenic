...
## Instrument before design

**MUST: give every test that can deadlock an explicit timeout.** A wedged DUT
without one is indistinguishable from a slow test, and the bill is hours.

**MUST: model memory with latency before claiming anything about a master's
pipelining.** Against a zero-latency slave, outstanding-burst depth is not
observable at all — a perfect implementation and a serial one measure the same.
A per-burst delay makes it a measurement; note that it also serialises bursts
in `cocotbext-axi`, so it is the wrong instrument for a throughput target.

**SHOULD: when a measurement accuses the design, check the instrument first.**
An unbound optional signal, a sink that is not actually waiting, a shim that
breaks the bus binder — each of these reads exactly like a broken DUT.
→ [`../docs/13-simulation-frameworks.md`](../docs/13-simulation-frameworks.md)

**MUST: run one simulator process per scenario when the DUT cannot be returned
to a cold state by reset.** Cases that fail sharing a process and pass alone are
the signature. It is also the faithful arrangement: on hardware a partial
reconfiguration destroys the partition outright.

**SHOULD NOT: replay a recorded trace in Bluesim.** The trace has to be compiled
into the BSV, which means a large ROM in simulation-only logic. Traces belong in
the cocotb tier, where Python feeds them and you can afford more of them.

## Two traps that look like design bugs

**MUST: warm up `2**htSizeLog2` cycles after reset before issuing table
commands.** The vendored cuckoo IP's `uram_bank.sv` clears its valid RAM one
entry per cycle after reset, so the wait **scales with your table size** — the
~2600-cycle figure belongs to the corpus default's 1024 entries and is eight
times too short for a 32768-entry table. Skip or under-size the warmup and early
inserts are silently wiped, every lookup misses, and the table looks broken when
it is not. Allow settling time after an
...
- [ ] Table warmup respected, scaled to **your** table size.
- [ ] Every test that can wedge carries a timeout.
- [ ] Any throughput claim measured against a full-bandwidth slave, not a
      serialising one.
