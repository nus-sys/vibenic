...
- **`mkCachedCuckooServer` needs `2**htSizeLog2` idle cycles after reset**
  before its first command — the vendored URAM banks clear their valid RAM one
  entry per cycle, so the wait scales with the table size (~2600 for the default
  1024 entries). It also never reports an insert failure, and it *hangs* rather
  than failing once the cuckoo path is exhausted; keep the load well under the
  placement limit by construction. → [`../docs/08-bsv-library-catalog.md`](../docs/08-bsv-library-catalog.md)
