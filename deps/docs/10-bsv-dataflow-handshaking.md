...
Patterns distilled from production BSV datapaths on this platform. The reusable
modules they describe are in [`../libs/bsv`](../libs/bsv); the worked code that
shows the same patterns composed into a datapath is
...
- `mkCounter` for in-flight accounting: conflict-free `.up` / `.down` /
  `.value`, the standard way to bound outstanding operations.
...
- `mkBufGPConnection(get, put, nStages)` — inserts N `mkFIFO` stages between a
  Get and a Put. The usual way to add slack and break long paths between
  blocks.
...
- `(* preempts = "ruleA, ruleB" *)` — if A is enabled it blocks B that cycle
  ...
  a transmit-completion rule preempts the lookup-start and receive-init group
  in a packet pipeline.
...
- **Self-draining sink**: when a response is unused,
  `rule flush; let _ <- srv.response.get; endrule`. Keeps the producer from
  stalling.
- **Manual round-robin arbiter**: a
...
- **Inline instrumentation without breaking dataflow**: wrap a FIFO in a module
  ...
  (a packet-length probe, for instance); or
