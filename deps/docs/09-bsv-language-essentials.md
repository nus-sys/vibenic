...
> **`Vector` packing runs the other way: element 0 is the LEAST significant
> bits.** The two conventions are opposite, and they meet in the one idiom that
> looks like it should be a no-op: `unpack(pack(myStruct))` into a
> `Vector#(n, Bit#(w))` puts the struct's **last** field in element 0. A counter
> block mapped to word offsets this way is readable at every offset and returns
> the wrong counter at every offset. Reverse explicitly (`Vector::reverse`) and
> say why in the code. Nothing catches this except a test that names one field
> and checks *that* field.
...
> **A value method has one port, and the scheduler counts *uses*, not uses that
> can co-occur.** Calling `tbl.resolve(...)` from a read path and a write path
> that are mutually exclusive by construction is still two uses, and `bsc`
> rejects it. The fix is not a second port: make the differing input an
> argument, call once, and select the result afterwards. That is also the
> cheaper hardware — one comparator tree instead of two.
...
> The same applies to two producers wired into one `Put`. It compiles; `bsc`
> emits **`G0010` — "treated as more urgent than"** — and picks a static winner.
> That is not arbitration: a busy source starves the other indefinitely. Use
> [`../libs/bsv/GPMux.bsv`](../libs/bsv/GPMux.bsv) when you want a policy.
> **Read `G0010` on every build.** A conflict the compiler resolves for you is
> still a design decision, just one you did not make.
...
## Structures that compile, simulate, and cannot close timing

These are the expensive ones, because functional testing cannot distinguish
them from the correct form. Only review or a build can.

> **`foldr`/`foldl` build a chain, not a tree.** Reducing 512 terms with
> `foldr(^, 0, terms)` is 512 levels of logic deep. It simulates correctly and
> passes every vector, and it will never close timing at 240 MHz. Use the
> reduction functions that emit a balanced tree — `reduceXor`, `reduceOr`,
> `reduceAnd`, `fold` over a `Vector` — and restructure the data so one of them
> applies. A GF(2) matrix-vector product, for instance, becomes `reduceXor(row
> & v)` per output bit once the matrix is stored row-major, at identical cycle
> count and correct depth.

> **A chained dynamic-index write is not a mux, it is a chain.** Scanning all
> 64 byte positions of a beat and accumulating into a register with a dynamic
> index emits 64 conditional updates, each feeding the next. It type-checks in
> seconds and then sends `bsc`'s code generator into tens of gigabytes of RSS
> and minutes of elaboration before it produces anything. Bound the extraction
> instead: if the field is at a known offset inside a known window, it is a
> handful of selects done once.

> **Match-and-select structures have a width-dependent ceiling.** A parallel
> compare followed by a priority encode is safe below about 16 ways at small
> widths, and 4–8 ways once the compared value reaches 128 bits or more. Past
> that, stage the logic and account for the extra cycle rather than lengthening
> the chain. Shrinking the table is a legitimate answer if timing does not
> close. Structure the check so the wide arithmetic happens **once** after the
> encode — sixteen parallel compares, a priority encode, then one bounds check,
> rather than sixteen bounds checks in series.
